...
public interface ConversationRepository extends CrudRepository<Conversation, Long>{
	...
	@Query("SELECT c FROM Conversation c WHERE c.customer = ?1 OR c.sale.user =?1")
	Iterable<Conversation> findByUser(User user);
	
	@Query("SELECT c FROM Conversation c WHERE (c.customer = ?1 or c.sale.user = ?1) and sale = ?2")
